<template>
  <div class='flex h-full' @mouseleave='handleResetActiveMenu'>
    ...
    <div class='relative h-full transition-width-300' :style='{
      width:appStore.mixSiderFixed?themeStore.sider.mixChildMenuWidth+"px":"0px"
    }'>
      <DarkModeContainer class='absolute-lt flex-vertical-stretch h-full nowrap-hidden transition-all-300 shadow-sm' :inverted='siderInverted' :style='{
        width:showDrawer?themeStore.sider.mixChildMenuWidth+"px":"0px"
      }'>
        <header class='flex-y-center justify-between' :style='{
          height:themeStore.header.height+"px"
        }'>
          <h2 class='text-primary pl-8px text-16px font-bold'>
            {{$t('system.title')}}
          </h2>
          <PinToggler :pin='appStore.mixSiderFixed' :class='{
            "text-white:88 !hover:text-bold":siderInverted
          }' />
        </header>
        <BaseMenu :dark-theme='sidernvrted' :menus='menus' />
      </DarkModeContainer>
